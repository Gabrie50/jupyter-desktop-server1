...
# Atualizar pacotes e instalar KDE Plasma 6 completo + utilitários
...
# Criar usuário padrão (igual NB_USER)
ARG NB_USER=jovyan
ARG NB_UID=1000
...
# Copia arquivos como xstartup, environment.yml etc
ADD . /opt/install
...
# Ativa ambiente Conda se necessário
RUN cd /opt/install && \
